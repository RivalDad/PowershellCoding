...
Function ModifyReg {
    ...
    # Deletes a Key
        ...
        Else{
            WriteToLog -Message "$path has already been removed" -color "Magenta"
        }
        ...
        Try {
            $CurrentPackage = Get-AppxPackage -Name $Bloat
                ...
                If ($CurrentPackage) {
                    ...
                    WriteToLog -Message "$Bloat already removed!" -color "Magenta"
            }
        } Catch { 
...
<FullDefaultLayoutTemplate 
    ...
    # Attempt to create it
        WriteToLog -Message "DefaultLayouts file does not exist.. Attempting to create" -color "Magenta"
    }
    else {
...
#>
	...
	ForEach ($crap in $RemoveableFolders) {
		...
		try {
			...
			WriteToLog -Message "$crap has already been removed for the 64bit" -color "Magenta"
		}
		...
		try {
			...
			WriteToLog -Message "$crap has already been removed for the 32bit" -color "Magenta"
		}
	}
