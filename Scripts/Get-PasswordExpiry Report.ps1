...
Get-MsolUser -All | select * | foreach{ 
 ...
 $Result=@{'Display Name'=$DisplayName;'User Principal Name'=$upn;'First Name'=$firstname;'Last Name'=$lastname;'Credentials Blocked'=$blockcredential;'Pwd Last Change Date'=$PwdLastChange;'Days since Pwd Last Set'=$PwdSinceLastSet;'Pwd Expiry Date'=$PwdExpiryDate;'Days since Expiry(-) / Days to Expiry(+)'=$PwdExpiresIn ;'Friendly Expiry Time'=$PwdExpireIn;'License Status'=$LicenseStatus}
 $Results= New-Object PSObject -Property $Result  
 $Results | Select-Object 'Display Name','First Name','Last Name','User Principal Name','Credentials Blocked','Pwd Last Change Date','Days since Pwd Last Set','Pwd Expiry Date','Friendly Expiry Time','License Status','Days since Expiry(-) / Days to Expiry(+)' | Export-Csv -Path $ExportCSV -Notype -Append 
}
