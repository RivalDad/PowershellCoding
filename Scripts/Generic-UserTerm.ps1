...
#This process is a 2parter. First terminate the AD account, then terminate the O365 mailbox. 
    ...
    try{
        ...
        #logwrite($errormessage) 
        write-host "The account wasn't hidden from the gal, please check the error logs for why"
    ...
    try{
        ...
        Write-host $success -foregroundcolor Green
        logwrite("The Description was updated Successfully.")
    } catch {
        logwrite($errormessage)
        write-host "Was not able to update the description. Please check the error logs for why" -foregroundcolor Green 
        start-process $logfile
        pause 
    }
    Start-process $Logfile
